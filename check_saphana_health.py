...
def function_check_M_SYSTEM_OVERVIEW(section, name, type):
    command_sql = "SELECT STATUS,VALUE FROM SYS.M_SYSTEM_OVERVIEW WHERE SECTION='" + \
        section + "' and NAME='" + name + "'"
...
if __name__ == '__main__':
    ...
    requiredNamed.add_argument(
        '--hostname', help="SAP HANA hostname", required=True)
    requiredNamed.add_argument(
        '--username', help="SAP HANA login", required=True)
    requiredNamed.add_argument(
        '--password', help="SAP HANA password", required=True)
    requiredNamed.add_argument(
    ...
    requiredNamed.add_argument(
        '--timeout', help="increase the default (60s) timeout")
